...
test('managed Lark credentials are encrypted at rest and revisioned', async (context) => {
  ...
  const created = await store.save({
    ...
    domain: 'feishu',
    verificationToken: 'callback-verification-token',
    encryptKey: 'callback-encryption-key',
    expectedRevision: 0,
  ...
  assert.equal(created.verificationTokenConfigured, true);
  assert.equal(created.encryptionKeyConfigured, true);
  ...
  assert.equal(persisted.includes('callback-verification-token'), false);
  assert.equal(persisted.includes('callback-encryption-key'), false);
